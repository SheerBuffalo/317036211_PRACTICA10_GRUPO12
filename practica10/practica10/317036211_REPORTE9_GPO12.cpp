...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, PosIni + glm::vec3(movKitX, 0, movKitZ));
		model = glm::rotate(model, glm::radians(rotKit), glm::vec3(0.0f, 1.0f, 0.0));
		model = glm::translate(model,glm::vec3(1.7f, 0.5f, 2.65f));
		model = glm::scale(model, glm::vec3(0.02, 0.02, 0.02));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::translate(model, PosIni + glm::vec3(movKitX, 0, movKitZ));
		model = glm::rotate(model, glm::radians(rotKit), glm::vec3(0.0f, 1.0f, 0.0));
		model = glm::translate(model, glm::vec3(-1.65f, 0.65f, 2.65f));
		...
		model = glm::translate(model, PosIni + glm::vec3(movKitX, 0, movKitZ));
		model = glm::rotate(model, glm::radians(rotKit), glm::vec3(0.0f, 1.0f, 0.0));
		model = glm::translate(model, glm::vec3(-1.75f, 0.75f, -2.9f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0));
		model = glm::scale(model, glm::vec3(0.02, 0.02, 0.02));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		LLanta.Draw(lightingShader);
...
void animacion()
{
	...
	if (circuito)
	{
		...
		if (recorrido6) 
		{
			//recorrido total Z = 90
			//recorrido total X = 90
			//m = x2-x1/z2-z1
			//x2= 0; x1= 90 (porque parte de ahi)
			//z2= 90; z1= 0
			//m = 0-90/90-0 = -1
			//por tanto por cada unidad que avanza Z, en una unidad disminuye X
			//angulo = arctan(-1) = -45
			rotKit = 315;

			//partiendo de deltaZ ya dado
			movKitZ += 0.1f; 

			//deltaX se obtiene como m*deltaZ=deltaX
			//deltaX = -1 * 0.1 = -0.1
			movKitX -= 0.1f; 


			if (movKitZ > 90 and movKitX < 0)
			{
				recorrido6 = false;
				recorrido4 = true;
